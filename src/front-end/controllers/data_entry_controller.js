...
export function initDataEntry() {
  ...
  async function mostrarPreview(file) {
    try {
      ...
      parsedRows = lines.map(
        (row) =>
          row
            .split(separator)
            .map((cell) => cell.trim().replace(/^["']|["']$/g, "")), // Limpiar comillas
      );
  ...
  function resetUploadState() {
    ...
    currentPage = 1;
    mainFileInput.value = "";
    uploadZoneText.innerText =
      "Arrastrá o hacé clic para buscar un archivo CSV";
  ...
  btnUpload.addEventListener("click", async (e) => {
    ...
    if (!backendType) {
      showMessage(
        "Por favor, selecciona una clase (tipo de archivo) antes de subir.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      if (success) {
        ...
        showMessage(
          "Error al cargar el archivo en el servidor. Revisa el formato.",
          "error",
        );
      }
    } catch (error) {
