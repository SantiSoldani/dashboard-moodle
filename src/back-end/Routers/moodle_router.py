...
from Models import Alumno, Semaforo, Usuario
...
async def lti_launch(request: Request, db: Session = Depends(server.get_db)):
    ...
    rol_moodle = form_data.get(
        "roles"
    )  # Return Instructor | Learner; Usamos el rol que da moodle para ahorrar pasos.
    ...
    if not usuario:
        ...
        if rol_moodle == "Learner":
            ...
            Semaforo.Post_Semaforo(semaforo_nuevo, db)
